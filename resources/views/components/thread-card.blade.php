...
<div
    ...
    <div class="flex justify-between items-center mb-4">
        <div class="flex items-center">
            <img src="{{ $thread->user->image ?? 'https://ui-avatars.com/api/?name=' . $thread->user->name . '&background=4817E4&color=fff&bold=true' }}"
                alt="User Logo" class="rounded-xl mr-2 border border-primary-hover" height="70" width=70">
            <div>
                ...
                <span class="text-gray-500">by <a wire:navigate
                        href="{{ route('users.threads', $thread->user->username) }}"
                        class="font-semibold text-primary hover:text-primary-hover">{{ $thread->user->name }}</a></span>
            </div>
        </div>
